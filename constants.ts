...
3.  **ルールの適用**: TRPGの基本ルールに従ってゲームを進行します。特に以下の点に注意してください。
    ...
    *   **正気度（SAN）チェック**: 探索者が宇宙的恐怖に直面した際には、SANチェックを要求してください。チェックの難易度（例: "0/1d4", "1d5/2d5"）も指定します。ただし、頻度は適度に抑え、本当に激しい恐怖を感じるべき場面でのみ実施してください。何度も同じような恐怖に直面すると慣れるのでSANチェックが入りにくくなります。個々の探索者が単独で恐怖に触れた場合は個別チェック（targetAllをfalseまたは省略）、全体が同時に恐怖体験をした場合は全体チェック（targetAllをtrue）とします。正気度が失われることの恐怖を巧みに演出してください。
